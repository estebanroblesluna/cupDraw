 ...
 */
@implementation SendBackwardCommand : Command
{
}
...
- (void) execute
{
	...
	if ([selectedFigures count] == 1) {
		...
		var insertIndex = [subviews indexOfObjectIdenticalTo: figure] - 1;
	    var otherFigure = [subviews objectAtIndex: insertIndex];

		[tool unselect: figure];
		[_drawing addSubview: figure positioned: CPWindowBelow relativeTo: otherFigure];
		[tool select: figure];
	}
}
@end
